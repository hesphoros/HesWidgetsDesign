...
Q_PROPERTY_DEFINE_MEMBER_CPP(HesAppBar, bool, IsStayTop)
Q_PROPERTY_DEFINE_MEMBER_CPP(HesAppBar, bool, IsDefaultClosed)
Q_PROPERTY_DEFINE_MEMBER_CPP(HesAppBar, bool, IsOnlyAllowMinAndClose)

HesAppBar::HesAppBar(QWidget* parent)
    : QWidget{parent}, d_ptr(new HesAppBarPrivate())
{
    Q_D(HesAppBar);
    d->_buttonFlags = HesAppBarType::RouteBackButtonHint | HesAppBarType::StayTopButtonHint | HesAppBarType::ThemeChangeButtonHint | HesAppBarType::MinimizeButtonHint | HesAppBarType::MaximizeButtonHint | HesAppBarType::CloseButtonHint;
...
#ifdef Q_OS_WIN
    HesWinShadowHelper::getInstance()->initDWMAPI();
    ...
    setObjectName("HesAppBar");
    setStyleSheet("#HesAppBar{background-color:transparent;}");
    d->_routeBackButton = new HesToolButton(this);
    d->_routeBackButton->setHesIcon(HesIconType::ArrowLeft);
    d->_routeBackButton->setFixedSize(40, 30);
    d->_routeBackButton->setEnabled(false);
    // 路由跳转
    connect(d->_routeBackButton, &HesIconButton::clicked, this, &HesAppBar::routeBackButtonClicked);

    // 导航栏展开按钮
    d->_navigationButton = new HesToolButton(this);
    d->_navigationButton->setHesIcon(HesIconType::Bars);
    ...
    // 展开导航栏
    connect(d->_navigationButton, &HesIconButton::clicked, this, &HesAppBar::navigationButtonClicked);

    // 设置置顶
    d->_stayTopButton = new HesToolButton(this);
    d->_stayTopButton->setHesIcon(HesIconType::ArrowUpToArc);
    d->_stayTopButton->setFixedSize(40, 30);
    connect(d->_stayTopButton, &HesIconButton::clicked, this, [=]() {
        this->setIsStayTop(!this->getIsStayTop());
    });
    connect(this, &HesAppBar::pIsStayTopChanged, d, &HesAppBarPrivate::onStayTopButtonClicked);
    ...
    //标题
    d->_titleLabel = new HesText(this);
    ...
    // 主题变更
    d->_themeChangeButton = new HesToolButton(this);
    d->_themeChangeButton->setHesIcon(HesIconType::MoonStars);
    d->_themeChangeButton->setFixedSize(40, 30);
    connect(d->_themeChangeButton, &HesIconButton::clicked, this, &HesAppBar::themeChangeButtonClicked);
    connect(hesTheme, &HesTheme::themeModeChanged, this, [=](HesThemeType::ThemeMode themeMode) {
        d->_onThemeModeChange(themeMode);
    });

    d->_minButton = new HesToolButton(this);
    d->_minButton->setHesIcon(HesIconType::Dash);
    d->_minButton->setFixedSize(40, 30);
    connect(d->_minButton, &HesIconButton::clicked, d, &HesAppBarPrivate::onMinButtonClicked);
    d->_maxButton = new HesToolButton(this);
    d->_maxButton->setIconSize(QSize(18, 18));
    d->_maxButton->setHesIcon(HesIconType::Square);
    d->_maxButton->setFixedSize(40, 30);
    connect(d->_maxButton, &HesIconButton::clicked, d, &HesAppBarPrivate::onMaxButtonClicked);
    d->_closeButton = new HesIconButton(HesIconType::Xmark, 18, 40, 30, this);
    ...
    connect(d->_closeButton, &HesIconButton::clicked, d, &HesAppBarPrivate::onCloseButtonClicked);
...
void HesAppBar::setAppBarHeight(int height)
{
    Q_D(HesAppBar);
...
int HesAppBar::getAppBarHeight() const
{
    Q_D(const HesAppBar);
    return d->_pAppBarHeight;
}

void HesAppBar::setCustomWidget(HesAppBarType::CustomArea customArea, QWidget* widget)
{
    Q_D(HesAppBar);
    ...
    switch (customArea)
    {
    case HesAppBarType::LeftArea:
    {
        d->_mainLayout->insertWidget(4, widget);
        break;
    }
    case HesAppBarType::MiddleArea:
    {
        d->_mainLayout->insertWidget(5, widget);
        break;
    }
    case HesAppBarType::RightArea:
    {
...
QWidget* HesAppBar::getCustomWidget() const
{
    Q_D(const HesAppBar);
    return d->_pCustomWidget;
}

void HesAppBar::setCustomWidgetMaximumWidth(int width)
{
    Q_D(HesAppBar);
...
int HesAppBar::getCustomWidgetMaximumWidth() const
{
    Q_D(const HesAppBar);
    return d->_pCustomWidgetMaximumWidth;
}

void HesAppBar::setIsFixedSize(bool isFixedSize)
{
    Q_D(HesAppBar);
...
bool HesAppBar::getIsFixedSize() const
{
    Q_D(const HesAppBar);
    return d->_pIsFixedSize;
}

void HesAppBar::setWindowButtonFlag(HesAppBarType::ButtonType buttonFlag, bool isEnable)
{
    Q_D(HesAppBar);
    if (isEnable)
    {
...
void HesAppBar::setWindowButtonFlags(HesAppBarType::ButtonFlags buttonFlags)
{
    Q_D(HesAppBar);
    d->_buttonFlags = buttonFlags;
    d->_routeBackButton->setVisible(d->_buttonFlags.testFlag(HesAppBarType::RouteBackButtonHint));
    d->_navigationButton->setVisible(d->_buttonFlags.testFlag(HesAppBarType::NavigationButtonHint));
    d->_stayTopButton->setVisible(d->_buttonFlags.testFlag(HesAppBarType::StayTopButtonHint));
    d->_themeChangeButton->setVisible(d->_buttonFlags.testFlag(HesAppBarType::ThemeChangeButtonHint));
    d->_minButton->setVisible(d->_buttonFlags.testFlag(HesAppBarType::MinimizeButtonHint));
    d->_maxButton->setVisible(d->_buttonFlags.testFlag(HesAppBarType::MaximizeButtonHint));
    d->_closeButton->setVisible(d->_buttonFlags.testFlag(HesAppBarType::CloseButtonHint));
}

HesAppBarType::ButtonFlags HesAppBar::getWindowButtonFlags() const
{
    return d_ptr->_buttonFlags;
}

void HesAppBar::setRouteBackButtonEnable(bool isEnable)
{
    Q_D(HesAppBar);
    d->_routeBackButton->setEnabled(isEnable);
}

void HesAppBar::closeWindow()
{
    Q_D(HesAppBar);
...
int HesAppBar::takeOverNativeEvent(const QByteArray& eventType, void* message, qintptr* result)
#else
int HesAppBar::takeOverNativeEvent(const QByteArray& eventType, void* message, long* result)
#endif
{
    Q_D(HesAppBar);
    ...
    case WM_NCACTIVATE:
    {
        if (HesWinShadowHelper::getInstance()->isCompositionEnabled())
        {
            return 0;
        }
        *result = true;
    ...
    case WM_LBUTTONDBLCLK:
    {
        ...
        postData.insert("WMClickType", HesAppBarType::WMLBUTTONDBLCLK);
        HesEventBus::getInstance()->post("WMWindowClicked", postData);
        return 0;
    }
    case WM_LBUTTONDOWN:
    {
        QVariantMap postData;
        postData.insert("WMClickType", HesAppBarType::WMLBUTTONDOWN);
        HesEventBus::getInstance()->post("WMWindowClicked", postData);
        return 0;
    }
    case WM_LBUTTONUP:
    {
        QVariantMap postData;
        postData.insert("WMClickType", HesAppBarType::WMLBUTTONUP);
        HesEventBus::getInstance()->post("WMWindowClicked", postData);
        return 0;
    }
    ...
    {
        QVariantMap postData;
        postData.insert("WMClickType", HesAppBarType::WMNCLBUTTONDOWN);
        HesEventBus::getInstance()->post("WMWindowClicked", postData);
...
bool HesAppBar::eventFilter(QObject* obj, QEvent* event)
{
    Q_D(HesAppBar);
    switch (event->type())
    {
...
#if (QT_VERSION >= QT_VERSION_CHECK(6, 5, 3) && QT_VERSION <= QT_VERSION_CHECK(6, 6, 1))
        HWND hwnd = (HWND)d->_currentWinID;
        HesWinShadowHelper::getInstance()->setWindowShadow(d->_currentWinID);
        DWORD style = ::GetWindowLongPtr(hwnd, GWL_STYLE);
