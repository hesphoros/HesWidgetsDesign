...
HesMenuStyle::HesMenuStyle(QStyle* style)
{
    _pMenuItemHeight = 32;
...
HesMenuStyle::~HesMenuStyle()
{
}

void HesMenuStyle::drawPrimitive(PrimitiveElement element, const QStyleOption* option, QPainter* painter, const QWidget* widget) const
{
    switch (element)
    {
    case QStyle::PE_PanelMenu:
    {
        ...
        hesTheme->drawEffectShadow(painter, option->rect, _shadowBorderWidth, 6);
        // 背景绘制
        ...
        painter->setPen(HesThemeColor(_themeMode, PopupBorder));
        painter->setBrush(HesThemeColor(_themeMode, PopupBase));
...
void HesMenuStyle::drawControl(ControlElement element, const QStyleOption* option, QPainter* painter, const QWidget* widget) const
{
    switch (element)
    {
    case QStyle::CE_MenuItem:
    {
        ...
        if (const QStyleOptionMenuItem* mopt = qstyleoption_cast<const QStyleOptionMenuItem*>(option))
        {
            if (mopt->menuItemType == QStyleOptionMenuItem::Separator)
            {
                ...
                painter->setBrush(HesThemeColor(_themeMode, BasicBaseLine));
            ...
            else
            {
                ...
                if (mopt->state.testFlag(QStyle::State_Enabled) && (mopt->state.testFlag(QStyle::State_MouseOver) || mopt->state.testFlag(QStyle::State_Selected)))
                {
                    ...
                    painter->setPen(Qt::NoPen);
                    painter->setBrush(HesThemeColor(_themeMode, PopupHover));
                ...
                {
                    painter->save();
                    painter->setPen(!mopt->state.testFlag(QStyle::State_Enabled) ? Qt::gray : _themeMode == HesThemeType::Light ? Qt::black
                                                                                                                                : Qt::white);
                    QFont iconFont = QFont("HesAwesome");
                    iconFont.setPixelSize(_pMenuItemHeight * 0.57);
                    painter->setFont(iconFont);
                    painter->drawText(QRectF(menuRect.x() + contentPadding, menuRect.y(), _iconWidth, menuRect.height()), Qt::AlignCenter, mopt->checked ? QChar((unsigned short)HesIconType::Check) : QChar((unsigned short)HesIconType::None));
                    painter->restore();
                }
                else
                {
                    QString iconText;
                    const HesMenu* menu = dynamic_cast<const HesMenu*>(widget);
                    if (menu)
                    {
                        QAction* action = menu->actionAt(menuRect.center());
                        if (action)
                        {
                            iconText = action->property("HesIconType").toString();
                        }
                    }
                    if (!iconText.isEmpty())
                    {
                ...
                if (mopt->menuItemType == QStyleOptionMenuItem::SubMenu)
                {
                    ...
                    painter->setFont(iconFont);
                    painter->drawText(QRect(menuRect.right() - 25, menuRect.y(), 25, menuRect.height()), Qt::AlignVCenter, QChar((unsigned short)HesIconType::AngleRight));
                    painter->restore();
                }
                painter->restore();
            }
...
int HesMenuStyle::pixelMetric(PixelMetric metric, const QStyleOption* option, const QWidget* widget) const
{
    switch (metric)
    {
...
QSize HesMenuStyle::sizeFromContents(ContentsType type, const QStyleOption* option, const QSize& size, const QWidget* widget) const
{
    switch (type)
    {
