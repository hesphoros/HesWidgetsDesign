...
#include "HesTheme.h"
HesToolButtonStyle::HesToolButtonStyle(QStyle* style)
{
    _pIsSelected = false;
    _pIsTransparent = true;
    _pExpandIconRotate = 0;
    _pBorderRadius = 4;
    _themeMode = hesTheme->getThemeMode();
    connect(hesTheme, &HesTheme::themeModeChanged, this, [=](HesThemeType::ThemeMode themeMode) {
        _themeMode = themeMode;
    });
}

HesToolButtonStyle::~HesToolButtonStyle()
{
}

void HesToolButtonStyle::drawComplexControl(ComplexControl control, const QStyleOptionComplex* option, QPainter* painter, const QWidget* widget) const
{
    switch (control)
    {
    case QStyle::CC_ToolButton:
    {
        ...
        if (const QStyleOptionToolButton* bopt = qstyleoption_cast<const QStyleOptionToolButton*>(option))
        {
            ...
            painter->setPen(_pIsTransparent ? Qt::transparent : HesThemeColor(_themeMode, BasicBorder));
            // 背景绘制
            if (bopt->state.testFlag(QStyle::State_Enabled))
            {
                if (bopt->state.testFlag(QStyle::State_Sunken))
                {
                    painter->setBrush(_pIsTransparent ? HesThemeColor(_themeMode, BasicPressAlpha) : HesThemeColor(_themeMode, BasicPress));
                ...
                else
                {
                    if (_pIsSelected)
                    {
                        painter->setBrush(_pIsTransparent ? HesThemeColor(_themeMode, BasicSelectedAlpha) : HesThemeColor(_themeMode, BasicHover));
                        painter->drawRoundedRect(toolButtonRect, _pBorderRadius, _pBorderRadius);
                    }
                    else
                    {
                        ...
                        {
                            painter->setBrush(_pIsTransparent ? HesThemeColor(_themeMode, BasicHoverAlpha) : HesThemeColor(_themeMode, BasicHover));
                            painter->drawRoundedRect(toolButtonRect, _pBorderRadius, _pBorderRadius);
                        }
                        else
                        {
                            if (!_pIsTransparent)
                            {
                                painter->setBrush(HesThemeColor(_themeMode, BasicBase));
                                painter->drawRoundedRect(toolButtonRect, _pBorderRadius, _pBorderRadius);
                                // 底边线绘制
                                painter->setPen(HesThemeColor(_themeMode, BasicBaseLine));
...
QSize HesToolButtonStyle::sizeFromContents(ContentsType type, const QStyleOption* option, const QSize& size, const QWidget* widget) const
{
...
void HesToolButtonStyle::_drawIndicator(QPainter* painter, const QStyleOptionToolButton* bopt, const QWidget* widget) const
{
    if (bopt->features.testFlag(QStyleOptionToolButton::MenuButtonPopup))
    {
        ...
        {
            painter->setBrush(HesThemeColor(_themeMode, BasicIndicator));
            QPainterPath path;
        ...
        // 指示器
        painter->setBrush(bopt->state.testFlag(QStyle::State_Enabled) ? HesThemeColor(_themeMode, BasicText) : HesThemeColor(_themeMode, BasicTextDisable));
    ...
    else if (bopt->features.testFlag(QStyleOptionToolButton::HasMenu))
    {
        ...
        int indicatorWidth = painter->fontMetrics().horizontalAdvance(QChar((unsigned short)HesIconType::AngleDown));
        ...
        painter->setPen(HesThemeColor(_themeMode, BasicText));
        ...
        painter->drawText(expandIconRect, Qt::AlignCenter, QChar((unsigned short)HesIconType::AngleDown));
        painter->restore();
    }
}

void HesToolButtonStyle::_drawIcon(QPainter* painter, QRectF iconRect, const QStyleOptionToolButton* bopt, const QWidget* widget) const
{
    if (bopt->toolButtonStyle != Qt::ToolButtonTextOnly)
    {
        QSize iconSize = bopt->iconSize;
        if (widget->property("HesIconType").toString().isEmpty())
        {
            // 绘制QIcon
        ...
        else
        {
            ...
            {
                painter->setPen(HesThemeColor(_themeMode, BasicTextDisable));
            }
            QFont iconFont = QFont("HesAwesome");
            switch (bopt->toolButtonStyle)
            {
            case Qt::ToolButtonIconOnly:
            {
                iconFont.setPixelSize(0.75 * std::min(iconSize.width(), iconSize.height()));
                painter->setFont(iconFont);
                painter->drawText(iconRect, Qt::AlignCenter, widget->property("HesIconType").toString());
                break;
            }
            case Qt::ToolButtonFollowStyle:
            case Qt::ToolButtonTextBesideIcon:
            {
                ...
                iconFont.setPixelSize(0.75 * std::min(iconSize.width(), iconSize.height()));
                painter->setFont(iconFont);
                painter->drawText(adjustIconRect, Qt::AlignCenter, widget->property("HesIconType").toString());
                break;
            }
            case Qt::ToolButtonTextUnderIcon:
            {
                ...
                painter->setFont(iconFont);
                painter->drawText(adjustIconRect, Qt::AlignHCenter, widget->property("HesIconType").toString());
                break;
            }
            default:
            {
...
void HesToolButtonStyle::_drawText(QPainter* painter, QRect contentRect, const QStyleOptionToolButton* bopt) const
{
...
qreal HesToolButtonStyle::_calculateExpandIndicatorWidth(const QStyleOptionToolButton* bopt, QPainter* painter) const
{
    // 展开指示器
    QSize iconSize = bopt->iconSize;
    painter->save();
