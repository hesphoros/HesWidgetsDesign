...
#include "../include/HesExponentialBlur.h"

HesMicaBaseInitObject::HesMicaBaseInitObject(HesApplicationPrivate* appPrivate, QObject* parent)
    : QObject(parent), _appPrivate(appPrivate)
{
}

HesMicaBaseInitObject::~HesMicaBaseInitObject()
{
}


void HesMicaBaseInitObject::onInitMicaBase(QImage img)
{
    // QColorDialog
    // 统一处理为1920*1080以节省空间
    img = img.scaled(QSize(1920, 1080), Qt::IgnoreAspectRatio, Qt::SmoothTransformation);

    QImage blurImage = HesExponentialBlur::doExponentialBlur(img, 500).toImage();
